...
class AIClient(private val config: PerceptionConfig) {
    ...
    suspend fun analyzeExternalCamera(base64Image: String): ExternalCamera {
        val startTime = System.currentTimeMillis()
        return withContext(Dispatchers.IO) {
            var result: ExternalCamera? = null
            try {
                withTimeout(10000L) {
                    ...
                    if (response.isSuccessful && responseBody != null) {
                        ...
                        if (jsonStart != -1 && jsonEnd != -1) {
                            ...
                            Log.d("AIClient", "外部摄像头AI分析完成，耗时: ${elapsedTime}ms")
                            result = ExternalCamera(
                ...
                val elapsedTime = System.currentTimeMillis() - startTime
                Log.w("AIClient", "外部摄像头AI分析超时，耗时: ${elapsedTime}ms")
            } catch (e: Exception) {
                val elapsedTime = System.currentTimeMillis() - startTime
                Log.e("AIClient", "外部摄像头AI分析异常，耗时: ${elapsedTime}ms, 错误: ${e.message}")
            }

            result ?: getMockExternalCamera()
        }
    }
    ...
    suspend fun analyzeInternalCamera(base64Image: String): InternalCamera {
        val startTime = System.currentTimeMillis()
        return withContext(Dispatchers.IO) {
            var result: InternalCamera? = null
            try {
                withTimeout(10000L) {
                    val jsonBody = JSONObject()
                    jsonBody.put("model", "qwen-vl-max")
                    ...
                    if (response.isSuccessful && responseBody != null) {
                        ...
                        if (jsonStart != -1 && jsonEnd != -1) {
                            ...
                            val elapsedTime = System.currentTimeMillis() - startTime
                            Log.d("AIClient", "内部摄像头AI分析完成，耗时: ${elapsedTime}ms")
                            result = InternalCamera(
                                mood = parsed.mood,
                ...
                val elapsedTime = System.currentTimeMillis() - startTime
                Log.w("AIClient", "内部摄像头AI分析超时，耗时: ${elapsedTime}ms")
            } catch (e: Exception) {
                val elapsedTime = System.currentTimeMillis() - startTime
                Log.e("AIClient", "内部摄像头AI分析异常，耗时: ${elapsedTime}ms, 错误: ${e.message}")
            }

            result ?: getMockInternalCamera()
        }
    }
